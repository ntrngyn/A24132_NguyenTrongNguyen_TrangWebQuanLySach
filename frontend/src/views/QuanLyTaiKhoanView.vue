...
<template>
  <div class="quanly-acc">
    ...
    <NhanVienForm v-if="showNhanVienForm" @submit="addNhanVien" @cancel="showNhanVienForm = false" />
    <DocGiaList v-if="activeTab === 'docgia'" :docGias="docGias" @deleteDocGia="deleteDocGia" />
    <NhanVienList v-if="activeTab === 'nhanvien'" :nhanViens="nhanViens" @deleteNhanVien="deleteNhanVien" />
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchDocGias() {
      try {
        const response = await axios.get('https://a24132-nguyentrongnguyen.onrender.com/api/docgia')
    ...
    async fetchNhanViens() {
      try {
        const response = await axios.get('https://a24132-nguyentrongnguyen.onrender.com/api/nhanvien')
    ...
    async deleteDocGia(id) {
      if (confirm('Bạn có chắc muốn xóa độc giả này?')) {
        try {
          await axios.delete(`https://a24132-nguyentrongnguyen.onrender.com/api/docgia/${id}`)
    ...
    async deleteNhanVien(id) {
      if (confirm('Bạn có chắc muốn xóa nhân viên này?')) {
        try {
          await axios.delete(`https://a24132-nguyentrongnguyen.onrender.com/api/nhanvien/${id}`)
    ...
    async addNhanVien(nhanvien) {
      try {
        ...
        const response = await axios.post(
          'https://a24132-nguyentrongnguyen.onrender.com/api/auth/register/nhanvien',
          nhanvien
        )
